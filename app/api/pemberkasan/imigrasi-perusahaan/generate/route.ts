...
import {
  fillImigrasiPerusahaanDocx,
  type ImigrasiPerusahaanFormData,
} from "@/app/admin-dashboard/pemberkasan/imigrasi-perusahaan/lib/fillImigrasiPerusahaanDocx";

export const runtime = "nodejs";

const TEMPLATE_REL =
  "app/admin-dashboard/pemberkasan/imigrasi-perusahaan/Dokumen_Imigrasi_Perusahaan(kosong).docx";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = body.data as ImigrasiPerusahaanFormData;

    const firstPesertaNama =
      data?.peserta?.find((p) => p?.nama?.trim())?.nama ||
      (data as { namaPeserta?: string })?.namaPeserta ||
      "";

    if (!firstPesertaNama && !data?.namaPerusahaanJp) {
      return NextResponse.json(
        { error: "Minimal 1 peserta atau namaPerusahaanJp wajib diisi" },
        { status: 400 },
      );
    }

    const filled = fillImigrasiPerusahaanDocx(readTemplate(), data);
    const safeName = String(firstPesertaNama || data.namaPerusahaanId || "dokumen")
      .replace(/[^\w\-]+/g, "_")
      .replace(/_+/g, "_")
      .slice(0, 40);
    const baseName = `Dokumen_Imigrasi_Perusahaan_${safeName || "dokumen"}`;
    ...
    if (format === "pdf") {
      ...
      const docxPath = path.join(tmp, `raftel-imigrasi-perusahaan-${stamp}.docx`);
      const pdfPath = path.join(tmp, `raftel-imigrasi-perusahaan-${stamp}.pdf`);
    ...
    console.error("[imigrasi-perusahaan generate]", message);
