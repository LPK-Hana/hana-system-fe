...
import { X, Search, CheckSquare, Square, Printer } from 'lucide-react';
import { type StudentEditData } from './StudentEditModal';

interface NafudaSelectModalProps {
  students: StudentEditData[];
  onClose: () => void;
  onPrint: (selectedStudents: StudentEditData[]) => void;
}

export default function NafudaSelectModal({ students, onClose, onPrint }: NafudaSelectModalProps) {
  ...
  const uniqueKelas = useMemo(() => Array.from(new Set(students.map(s => s.tingkatan_pembelajaran).filter(a => a && a !== '-'))), [students]);
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (filterKelas !== 'Semua') {
      result = result.filter(s => s.tingkatan_pembelajaran === filterKelas);
    }
    if (searchTerm) {
    ...
    return result;
  }, [students, searchTerm, filterAngkatan, filterKelas]);
    ...
    <div className="fixed inset-0 z-[150] bg-black/45 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white border border-gray-200 shadow-2xl flex flex-col h-[85vh]">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/50">
          <div>
            <h2 className="text-lg font-serif text-gray-900">Pilih Siswa untuk Nafuda</h2>
            <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">Silakan pilih siswa yang akan dicetak name tag-nya.</p>
          </div>
        ...
        <div className="p-4 border-b border-gray-100 flex flex-col gap-3 bg-white">
          <div className="flex items-center gap-3">
            <select
              ...
              className="px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:border-purple-500"
            >
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-sm border border-gray-300 focus:outline-none focus:border-purple-500 rounded-md"
              />
            </div>
            <button
              onClick={toggleAll}
              className="flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-purple-600 transition-colors whitespace-nowrap"
            >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                        ...
                        <span>•</span>
                        <span className="bg-gray-100 px-1.5 py-0.5 rounded text-[10px] uppercase">{student.angkatan}</span>
                      </div>
                    </div>
                  </label>
                );
        ...
        <div className="p-4 border-t border-gray-100 bg-white flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Terpilih: <span className="font-semibold text-purple-600">{selectedIds.size}</span> siswa
          </div>
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2 text-sm font-medium bg-purple-600 text-white hover:bg-purple-700 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Printer size={16} />
              Cetak Nafuda
            </button>
